...
def run_trial(trial: optuna.Trial, network: str, data_path: str, output_root: str,
    ...
    if experiment:
        ...
        experiment.add_tag(f"optuna_{network}_no_gan")
        experiment.end()
...
def main():
    ...
    print(f"[{args.network}/no_gan] storage={storage} study={study_name}")
    print(f"[{args.network}/no_gan] existing trials: {len(study.trials)}, target: {args.n_trials}")
    ...
    print(f"\n[{args.network}/no_gan] Pareto front:")
    for t in study.best_trials:
        print(
            f"  trial {t.number:>4}  PSNR={t.values[0]:.3f}dB  comp_ratio={t.values[1]:.4f}  "
            f"params={t.user_attrs.get('generator_params', '?')}  "
            f"rmse={t.user_attrs.get('final_rmse_celsius', '?')}"
        )
